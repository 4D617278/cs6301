...
Proof.
  intros p. destruct p as [f s]. simpl. reflexivity.
Qed.
(** [] *)
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h::t => (count h s1 <=? count h s2) && (included t s2)
  end.
...
Theorem bag_count_sum : forall n s1 s2,  
  count n (sum s1 s2) = count n s1 + count n s2.
Proof.
  intros n s1 s2.
  induction s1 as [| n' s1' IHs'].
  - simpl. reflexivity.
  - simpl. destruct (n' =? n).
  + simpl. rewrite <- IHs'. reflexivity.
...
(* The type baz has Baz1 + 2 * Baz2 elements. *)
